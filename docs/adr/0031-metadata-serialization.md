...
- **作者**: ai-hub core team
- **日期**: 2026-07-19
- **状态**: **Accepted** ✅ (ChatGPT ADR 审核 9.6/10 APPROVED with 5 adopted revisions)
- **依赖**:
...
- **ChatGPT ADR 审核**: 9.6/10 APPROVED — `docs/reviews/0031-adr-chatgpt-review.md`
- **采纳修订** (5 项, ChatGPT 9.6/10):
  - **R1** (Blocking-2 采纳): §3.2 明确 `serialize_xxx()` 是 canonical implementation, `to_dict()` 仅 facade
  - **R2** (建议2 采纳): `METADATA_SCHEMA_VERSION` → `FUTURE_METADATA_SCHEMA_VERSION` 减少误解
  - **R3** (建议3 确认): `StageRegistry.to_dict()` 用 `serialize_stage_info()` (canonical 而非 facade)
  - **R4** (Q8 采纳): §6 测试策略增加 `test_json_round_trip` + `test_no_mutation_property`
  - **R5** (commit 顺序采纳): §7 实施计划明确 5 个独立 commits
- **Blocking-1 (hallucination)**: ChatGPT 提到 ADR 代码 typo (`has_side_effeways_run_after_stop` / `serialize_desccriptor` / `return {"stage":,}`)，全文搜索确认**不存在** (LLM hallucination)，无需修改
...
# R2 (ChatGPT 9.6/10): 重命名 METADATA_SCHEMA_VERSION → FUTURE_METADATA_SCHEMA_VERSION 减少误解
...
**R1 (ChatGPT 9.6/10 Blocking-2 采纳) — Canonical API 约束**:
> `serialize_xxx()` 是 **canonical serialization implementation**，`to_dict()` methods **仅是 facade** (convenience wrapper)。
>
> 此约束避免未来 `StageDescriptor.to_dict()` 内出现业务逻辑（应永远 delegate 到 `serialize_descriptor`）。
>
> 实施: 每次 `to_dict()` method 必须为单行 delegate (`return serialize_xxx(self)`)，禁止内联序列化逻辑。
...
9. **TestRoundTripAndNoMutation** (4 tests, R4 采纳 ChatGPT Q8 建议)
   - `test_json_round_trip_descriptor` — `json.loads(to_json(serialize_descriptor(d))) == serialize_descriptor(d)`
   - `test_json_round_trip_stage_info` — StageInfo round-trip 稳定
   - `test_json_round_trip_runtime_metadata` — RuntimeMetadata round-trip 稳定
   - `test_no_mutation_property` — `copy.deepcopy(rm); serialize_runtime_metadata(rm); assert rm == before`
