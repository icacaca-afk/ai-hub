# ADR-0030 Registry Introspection — ChatGPT 审核 prompt

请审核以下 ADR-0030 (V1.0.9 Registry Introspection) 草稿。

## 审核范围

- ADR 文件: `docs/adr/0030-registry-introspection.md` (~580 行)
- 依赖: ADR-0029 Stage Registry (V1.0.8 Accepted Rev1 9.72/10)
- 后续: ADR-0031 Metadata Serialization (V1.0.9 MUST ②)

## 项目背景

ai-hub 是本地 AI Runtime, 从 Provider Router 演进为 AI Operating System / Agent Runtime。

V1.0.x 已完成:
- V1.0.1 ADR-0021 ExecutionPipeline (9.95/10)
- V1.0.3 ADR-0022 RetryStage (9.9/10)
- V1.0.3 ADR-0023 CheckpointStage (9.9/10)
- V1.0.4 ADR-0024 ConditionStage (9.92/10)
- V1.0.5 ADR-0025 PipelineHooks (9.88/10)
- V1.0.6 ADR-0026 StageDescriptor (9.95/10)
- V1.0.7 ADR-0027 RuntimeMetadata (9.88/10)
- V1.0.8 ADR-0028 Metadata Access API (9.94/10)
- V1.0.8 ADR-0029 Stage Registry (ADR 9.93/10 + 代码 9.72/10 Rev1)

V1.0.8 Rev1 关键约束 (代码审核 9.72/10 采纳):
- **R1**: Registry stores discoverable Stage definitions, not guaranteed executable runtime instances
- **R2**: default_pipeline(router, *, store=None, registry=None) -> ExecutionPipeline (非零参)
- **R3**: 三阶段 DI 模型 (Registry phase / Factory phase / Execution phase)
- **R4**: RouteStage/CheckpointStage misuse guard (router=None / _NullStore stub → RuntimeError)

## ADR-0030 核心设计

### 1. 6 个 Introspection API

```python
class StageRegistry:
    def info(name) -> Optional[StageInfo]              # 完整信息 (descriptor + source + requires + registered_at)
    def describe_all() -> Dict[str, StageDescriptor]   # 所有 descriptor
    def summary() -> List[StageSummary]                # 简短摘要 (CLI 友好)
    def list_builtin() -> List[str]                    # source="builtin"
    def list_third_party() -> List[str]                # source="third_party"
    def find_stages_needing(*deps) -> List[str]        # 按 runtime dep 查询
    def to_dict() -> Dict[str, Any]                    # 序列化 (MCP / WebUI / CLI)
    def to_json(indent=2) -> str                       # JSON 字符串
```

### 2. StageInfo / StageSummary 数据结构
...
V1.0.8 旧调用 `register(stage)` / `register(stage, replace=True)` 完全兼容。

### 4. Built-in Stage 注册扩展

```python
def _register_builtin_stages(registry: StageRegistry) -> None:
    registry.register(RouteStage(router=None), source="builtin", requires=("router",))
    registry.register(RetryStage(), source="builtin", requires=())
    registry.register(CheckpointStage(store=_NullStore()), source="builtin", requires=("store",))
    registry.register(ConditionStage(...), source="builtin", requires=())
    registry.register(MetricsStage(), source="builtin", requires=())
```

### 5. CLI 集成 (V1.0.10 实施, V1.0.9 仅 API)
...
$ ai-hub stage dump --json
```

## 审核重点 (8 个问题)

1. **StageInfo 字段是否合适?** source / requires / registered_at 三个字段是否够用? 是否应该加 `last_used_at` / `invocation_count` (Runtime 观测)?

2. **requires 字符串语义?** `("router",)` / `("store",)` 是否应该用 Enum (`Dep.ROUTER`)? V1.x 字符串 vs V2 Enum 的转换成本?

3. **source 字符串 vs Enum?** "builtin" | "third_party" | "test" 是否够用? 是否需要 "deprecated" / "experimental"?
...
5. **CLI 应该在 V1.0.9 还是 V1.0.10?** V1.0.9 仅 API + 数据结构, V1.0.10 实施 CLI。是否合理?

6. **find_stages_needing(*deps) 语义?** 当前是 "Stage requires 所有这些 deps" (issubset)。是否应该提供 "任一 dep" (intersection) 模式?

7. **registered_at 是否启用?** V1.0.9 启用 (datetime.now()), 还是延后到 V1.1?
...
1. **架构方向**: 是否解决 V1.0.8 → V1.0.9 演进的痛点?
2. **API 一致性**: 6 个 Introspection API 是否与 V1.0.8 8 核心 API 风格一致?
3. **向后兼容**: V1.0.8 测试是否全通过? (V1.0.9 register() 扩展签名)
4. **数据结构**: StageInfo / StageSummary frozen dataclass 是否合理?
5. **替代方案**: 6 个替代方案拒绝是否合理?
6. **测试策略**: 25+ tests 覆盖是否充分?
7. **实施计划**: 5 阶段 (ADR → 代码 → 测试 → 审核 → ADR-0031) 是否合理?
8. **V1.0.10 演化**: Pipeline Introspection / Predicate API / CLI 留待 V1.0.10 是否合理?

## 期望评分

**9.5+/10** (V1.0.8 ADR-0029 ADR 审核 9.93/10 + 代码审核 9.72/10, 本 ADR 类似范围)
...
```
ADR-0030 Registry Introspection Review
总评分: X.XX / 10
...
请审核 `docs/adr/0030-registry-introspection.md` 完整内容 (~580 行)。
...
1. **6 个 Introspection API**:
   ...
   - `summary() -> List[StageSummary]` — 简短摘要 (name / role / capabilities / source)
   - `list_builtin() / list_third_party() -> List[str]` — 按 source 分组
   - `find_stages_needing(*deps) -> List[str]` — 按 runtime dep 查询
   - `to_dict() / to_json() -> ...` — 序列化 Registry 状态
   - `info(name) -> StageInfo` — 单个 Stage 详细信息 (descriptor + source + deps)
...
class StageInfo:
    ...
    比 StageDescriptor 多:
      ...
      - registered_at: datetime (V1.0.9 启用)
    ...
    source: str = "third_party"           # "builtin" | "third_party" | "test"
    requires: Tuple[str, ...] = ()         # runtime dep names
    registered_at: Optional[datetime] = None  # V1.0.9 启用
...
class StageRegistry:
    ...
    def find_stages_needing(self, *deps: str) -> List[str]:
        """列出所有 requires 包含指定 deps 的 Stage name (V1.0.9).
...
### 3.4 为什么 `registered_at` 是 `Optional[datetime]`?

- V1.0.9 启用, 但保留 None 兼容路径 (e.g. 测试 fixture 不关心时间)
- V1.1 评估: 加 `last_used_at` / `invocation_count` (Runtime 观测)
- V2 评估: 加 `version_history` (multi-version registration)

### 3.5 为什么 `to_dict()` 把 `descriptor` 也展开?
